...
PARAMS = {
    "test": {
        ...
        "baseline_products_table":"baseline_qc.expected_forecasted_products",
        "prediction_dates": {
        ...
        "products_s3_path": "s3://test-q-forecasting-artifacts/baseline/store_products/store_products_list.csv",
    ...
    "prod": {
        ...
        "products_s3_path": "s3://prod-q-forecasting-artifacts/baseline/store_products/store_products_list.csv",
        "step_1_data_fetch_days_limit": 200,
        "last_year_dates_lag": 364,
        "default_product_number": 99999,
        "lag_after_high_low_removal": 3,
...
for batch_no,batch in zip(range(1,batch_count+1),batches):
    ...
    if metric != 'itemcount':
        ...
        products.persist()
        products_pd = products.groupBy('location_num').count()\
                              .withColumn('generation_date',F.lit(str(datetime.now(EST).date())))\
                              .toPandas()
        products_df = products_df.append(products_pd)
    ...
    df1 = df.join(dates,on=['location_num','business_date'],how='right')\
             .groupBy(['location_num','product','prediction_date','interval',\
                        'lookup_weeks','forecast_avg_dr'])\
             .agg(
...
products_df.to_csv(PARAMS[ENV]['products_s3_path'],header=True, index=False)
products_spark = spark.createDataFrame(products_df).withColumnRenamed("count","products")
products_spark.write.jdbc(
    secret["mysql-url"],
    PARAMS[ENV]["baseline_products_table"],
    mode="append", # "append", "overwrite", "ignore", "error" or "errorifexists"
    properties={ 'user' : secret["username"], 'password' : secret["password"] }
)
print("store,products_count list stored in S3")
...
#         s3_path = f"s3://{PARAMS[ENV]['bucket']}/{PARAMS[ENV]['forecast_output_path'][metric]}/store_num={store}/"
#         # print(s3_path)
