select aa.weekday,aa.sales_total from
    (select a.business_date,weekday(a.business_date) weekday,sum(a.sales_sub_total) sales_total from
            (
            select location_num,business_date,sales_sub_total,generation_date from ml_preprod.initial_dollarsalesandtranscount_daily
