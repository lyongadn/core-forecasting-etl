       ...
       CASE 
         WHEN actual_items < expected_items 
               OR (SELECT Min(business_date) 
                   FROM   redundancy.__itemcount_forecast_table__ 
                   WHERE  generation_date = (SELECT Max(generation_date) 
                                             FROM 
                          redundancy.__itemcount_forecast_table__ 
                                             WHERE  location_num = __store_num__ 
                                            ) 
                          AND location_num = __store_num__) <> 
                  Date_add((select max(generation_date) from ui_preprod.daily_dollarsales_lstm_14days_ahead where location_num =__store_num__), INTERVAL __redundancy_forecast_from__ day) 
               OR (SELECT Max(business_date) 
                   ...
                   WHERE  generation_date = (SELECT Max(generation_date) 
                          ...
                          AND location_num = __store_num__) <> 
                  Date_add((select max(generation_date) from ui_preprod.daily_dollarsales_lstm_14days_ahead where location_num =__store_num__), INTERVAL __redundancy_forecast_to__ day) THEN 0 
