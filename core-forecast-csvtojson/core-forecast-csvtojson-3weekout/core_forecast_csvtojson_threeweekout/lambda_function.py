...
def upload_to_s3(prod_bucket, upload_path, local_path, gen_date, conf):
    ...
    """
    forecast = 'LSTM-30DaysAhead'
    if (conf.get_forecast_type() == 'baseline'):
        forecast = 'Baseline-30DaysAhead'
...
def get_location_data_limits(connection, conf, weekday):
    ...
    query_dollarsales = open('core_forecast_csvtojson_3weekout/data_limits.sql', 'r').read()
    ...
    query_transcount = open('core_forecast_csvtojson_3weekout/data_limits.sql', 'r').read()
    ...
    query_itemcount = open('core_forecast_csvtojson_3weekout/data_limits.sql', 'r').read()
    ...
    query_ingredient = open('core_forecast_csvtojson_3weekout/data_limits.sql', 'r').read()
...
def lambda_handler(event, context):
    ...
    with open('core_forecast_csvtojson_3weekout/config.json') as config_params:
    ...
    query_dates = open('core_forecast_csvtojson_3weekout/get_dates.sql', 'r')
    ...
    for dates in business_dates:
        ...
        lastyear_date = open('core_forecast_csvtojson_3weekout/get_last_year_date.sql', 'r')
        ...
        daily_dollartranscount = open('core_forecast_csvtojson_3weekout/daily_dollartranscount.sql', 'r').read()
        ...
        if len(store_list) < 1:
            ...
            qry_15min_dollartranscount = open('core_forecast_csvtojson_3weekout/15min_dollartranscount.sql', 'r').read()
            ...
            qry_15min_itemcount = open('core_forecast_csvtojson_3weekout/15min_itemcount.sql', 'r').read()
            ...
            qry_15min_ingredient = open('core_forecast_csvtojson_3weekout/15min_ingredient.sql', 'r').read()
