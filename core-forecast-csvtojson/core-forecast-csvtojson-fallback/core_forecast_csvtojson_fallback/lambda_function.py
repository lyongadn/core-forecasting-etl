...
def upload_to_s3(prod_bucket, upload_path, local_path, gen_date, forecast):
    """
    ...
    # if (conf[forecast_type]get_forecast_type() == 'baseline'):
...
# def upload_alerts_s3(conf, alerts_df, gen_date_forecast):
#     local_path = conf[forecast_type]get_alert_local_path().replace('__location_num__',str(store_num))
#     alerts_df.to_parquet(local_path,index=False)
#     print(alerts_df.head())
#     path_alert = conf[forecast_type]get_alert_path().replace('__gen_date__',gen_date_forecast.split('T')[0])
#     upload_path = f"{path_alert+str(store_num)+'_'+str(datetime.datetime.today().date())}.parquet"
#     print(upload_path)
#     s_3 = boto3.client('s3')
#     s_3.upload_file(local_path, conf[forecast_type]get_alert_bucket(), upload_path)
#     return True
...
#     keys = kms_manager.get_secret_value(SecretId=conf[forecast_type]get_secret_key_slack())
#     credentials = json.loads(keys['SecretString'])
#     slack_channel = conf[forecast_type]get_slack_channel_QC()
#     hook_url = credentials['hook_url']

#     slack_message = {
#         'channel': slack_channel,
#         'text': message,
#         "username": conf[forecast_type]get_slack_username(),
#         "icon_emoji": conf[forecast_type]get_slack_emoji()
#     }
#     # print(slack_message)
...
def get_location_data_limits(connection, conf, weekday, store_num):
    '''Compute the Forecast limits based the data available in past 20 days'''
    query_dollarsales = open('core_forecast_csvtojson_fallback/data_limits.sql', 'r').read()
    query_dollarsales = query_dollarsales%(store_num, weekday)
    query_dollarsales = query_dollarsales.replace('weekday_stats', \
        conf['dollarsales_weekday_table_name'])
    dollarsales_data = execute_query(query_dollarsales, connection, \
        conf['database'], store_num, conf['weekly_stats_table_schema'].split(','))
    

    query_transcount = open('core_forecast_csvtojson_fallback/data_limits.sql', 'r').read()
    query_transcount = query_transcount.replace('weekday_stats', \
        conf['transcount_weekday_table_name'])

    transcount_data = execute_query(query_transcount%(store_num, weekday), \
     connection, conf['database'], store_num, conf['weekly_stats_table_schema'].split(','))

    query_itemcount = open('core_forecast_csvtojson_fallback/data_limits.sql', 'r').read()
    query_itemcount = query_itemcount.replace('weekday_stats', \
        conf['itemcount_weekday_table_name'])

    itemcount_data = execute_query(query_itemcount%(store_num, weekday), \
     connection, conf['database'], store_num, conf['weekly_stats_table_schema'].split(','))

    query_ingredient = open('core_forecast_csvtojson_fallback/data_limits.sql', 'r').read()
    query_ingredient = query_ingredient.replace('weekday_stats', \
        conf['ingredient_weekday_table_name'])

    ingredient_data = execute_query(query_ingredient%(store_num, weekday), \
     connection, conf['database'], store_num, conf['weekly_stats_table_schema'].split(','))
...
def json_qc(connection, conf, business_date, location_num, weekday):
    ...
    metric_10daysahead, dfitem_json160001, dfingredient_json1, gen_date = reading_json( \
                                            conf['json_local_path'], business_date, location_num)
    gen_date_forecast= gen_date
    gensummarydate = readin_Jsonsummary(conf['summaryjson_local_path'], business_date,location_num)
    ...
    transcountlimits_data, dollarsaleslimits_data, itemquantity_limits_data, ingredientlimits_data\
                            = get_location_data_limits(connection, conf, weekday, location_num)
    ...
    flag_final_dollars, flag_greater_limit_dollars = quality_check(dfmetric_json, dfmetric_json, \
        ...
        location_num, business_date, dollarsaleslimits_data, alert)
    ...
    flag_final_trans, flag_greater_limit_trans = quality_check(dfmetric_json, dfmetric_json, \
        ...
        location_num, business_date, transcountlimits_data, alert)
    ...
    flag_final_item, flag_greater_limit_item = quality_check(dfitem_json, dfitem_json160001, \
     ...
     location_num, business_date, itemquantity_limits_data, alert)
    ...
    flag_final_ingre, flag_greater_limit_ingre = quality_check(dfingredient_json, dfingredient_json1, \
     ...
     location_num, business_date, ingredientlimits_data, alert)
...
def lambda_handler(event, context):
    ...
    store_num = event['store_num']
    forecast_type = event['forecast_type']
    
    with open('core_forecast_csvtojson_fallback/config.json') as config_params:
    ...
    # database = conf[forecast_type]['database']
    # if (conf[forecast_type]get_forecast_type() == 'baseline'):
    #     database = conf[forecast_type]get_baseline_database()

    print("Store is", store_num)
    print("Forecast Type is", forecast_type)
    
    database = conf['database']
    connection = pymysql.connect(host=conf['custom_host_link'],
                                 user=credentials['username'],
                                 password=credentials['password'],
                                 db=conf['database'])
    LOGGER.info("Successfully connected to Aurora")

    business_dates = event['business_date']
    # query_dates = open('get_dates.sql', 'r')
    # dates = query_dates.read()
    # business_dates = get_business_date(dates, connection, database, \
    #     conf[forecast_type]get_15min_dollar_table(), store_num, 'fetchdates')
    # print("Dates are",business_dates)
    path_alert = conf[forecast_type]['alert_upload_path']
    
    for dates in business_dates:
        
        lastyear_date = open('core_forecast_csvtojson_fallback/get_last_year_date.sql', 'r')
        lastyear_dates = lastyear_date.read()
        lastyear_dates = lastyear_dates.replace('%s', str(dates))
        lastyear_date_df = get_business_date(lastyear_dates, connection, conf['database'], \
        conf[forecast_type]['15min_dollar_table_name'], store_num, 'last_year_date')
        print((lastyear_date_df['business_date'][0]))
        lastyear_date = (lastyear_date_df['business_date'][0])

        daily_dollartranscount = open('core_forecast_csvtojson_fallback/daily_dollartranscount.sql', 'r').read()
        daily_dollartranscount = daily_dollartranscount.replace('15min_dollar_table',\
            conf[forecast_type]['15min_dollar_table_name']).replace('final_table',\
            conf[forecast_type]['daily_final_table_name']).replace('%s', str(dates)).replace('date_last_year',\
             str(lastyear_date))
        dollartranscount_daily = execute_query(daily_dollartranscount, connection, \
            database, store_num, conf['daily_dollartranscount_table_schema'].split(','), \
            conf['to_str_daily_dollartranscount_column_names'].split(','))
        ...
        if len(store_list) < 1:
            ...
            LOGGER.info('Daily forecast for dollarsales and transcount is done for '+store_num)
            
            qry_15min_dollartranscount = open('core_forecast_csvtojson_fallback/15min_dollartranscount.sql', 'r').read()
            qry_15min_dollartranscount = qry_15min_dollartranscount.replace('15min_dollar_table', \
                conf[forecast_type]['15min_dollar_table_name']).replace('final_table_15min', \
                conf[forecast_type]['15min_final_table_name']).replace('%s', str(dates)).replace('date_last_year',\
             str(lastyear_date))
            dollartranscount_15min = execute_query(qry_15min_dollartranscount, connection, \
                database, store_num, \
                conf['15min_dollartranscount_table_schema'].split(','), \
                conf['to_str_15min_dollartranscount_column_names'].split(','))
            LOGGER.info('15min forecast for dollarsales and transcount is done for '+store_num)

            qry_15min_itemcount = open('core_forecast_csvtojson_fallback/15min_itemcount.sql', 'r').read()
            ...
            qry_15min_itemcount_final = qry_15min_itemcount_final.replace('15min_itemcount_table', \
                conf[forecast_type]['15min_itemcount_table_name'])
            ...
            itemcount_15min = execute_query(qry_15min_itemcount_final, connection, \
                database, store_num, conf['15min_itemcount_table_schema'].split(','), \
                conf['to_str_15min_itemcount_column_names'].split(','))
            LOGGER.info('15min forecast for Itemcount is done for'+store_num)

            qry_15min_ingredient = open('core_forecast_csvtojson_fallback/15min_ingredient.sql', 'r').read()
            qry_15min_ingredient = qry_15min_ingredient.replace('15min_ingredient_table', \
                conf[forecast_type]['15min_ingredient_table_name'])
            ...
            ingredient_15min = execute_query(qry_15min_ingredient, connection, \
                database, store_num, conf['15min_ingredient_table_schema'].split(','), \
                conf['to_str_15min_ingredient_column_names'].split(','))
            LOGGER.info('15min forecast for Ingredient is done for'+store_num)
            ...
            with open(conf['summaryjson_local_path']+date+'.json', 'w') as outfile:
                json.dump(forecast_main_summary, outfile)
                
            LOGGER.info('Summary JSON is generated and uploaded in temp folder for'+store_num)
            ...
            with open(conf['json_local_path']+date+'.json', 'w') as outfile:
                print((conf['json_local_path']+date+'.json'))
                json.dump(forecast_main, outfile)
            LOGGER.info('Forecast JSON is generated for '+store_num)

            path_alert = f"{path_alert}{str(store_num)}_{str(dates)}_fallback_alerts.csv"   
    ...
    for dates in range(len(business_dates)):
        try :
            ...
            """
            business_dt = datetime.datetime.strptime(business_dates[dates], "%Y-%m-%d")
            date = business_dt.strftime("%m-%d-%Y")
            gen_date_forecast = reading_json(conf['json_local_path'], date, str(store_num))
            gen_date_summary_forecast = readin_Jsonsummary(conf['summaryjson_local_path'], date, str(store_num))
            
            # status, date, flag_greater_limit, gen_date_forecast, \
            # gen_date_summary_forecast, alert= json_qc(connection, conf, \
            #             business_dt.strftime("%m-%d-%Y"), \
            #             str(store_num), business_dt.weekday())
            # alerts.append(alert)
            ...
            """
            upload_to_s3(conf['json_bucket'], \
            conf['upload_path']+dollartranscount_daily['StoreNumber'][0]+'/'+'demand-'+\
            date+'.json', \
            conf['json_local_path']+date+'.json', gen_date_forecast, f"Baseline-{forecast_type}-ahead")
            ...
            """
            upload_to_s3(conf['json_bucket'], \
            conf['summaryjson_upload_path']+dollartranscount_daily['StoreNumber'][0]+'/'+'demand-'+\
            business_dt.strftime("%m-%d-%Y")+'.json', \
            conf['summaryjson_local_path']+date+'.json', gen_date_summary_forecast, f"Baseline-{forecast_type}-ahead")
            LOGGER.info("Summary JSON uploaded")
                
            # else:
            #     upload_to_s3(conf['prod_bucket'], \
            #     conf['upload_path_test']+dollartranscount_daily['StoreNumber'][0]+'/'+'demand-'+\
            #     business_dt.strftime("%m-%d-%Y")+'.json', \
            #     conf['json_local_path']+date+'.json', gen_date_forecast, f"Baseline-{forecast_type}-ahead")
            #     LOGGER.info("Forecast JSON is not in limits and uploaded to secondary(prod-q-forecasting) bucket")
                
            #     upload_to_s3(conf['prod_bucket'], \
            #     conf['summaryjson_upload_path_test']+dollartranscount_daily['StoreNumber'][0]+'/'+'demand-'+\
            #     business_dt.strftime("%m-%d-%Y")+'.json', \
            #     conf['summaryjson_local_path']+date+'.json', gen_date_summary_forecast, f"Baseline-{forecast_type}-ahead")
            #     LOGGER.info("JSON QC uploaded to test bucket")
                
            LOGGER.info("JSON upload completed for"+str(store_num))
    ...
    # print(alerts_df)
    # alerts_df.to_csv(f"s3://{conf['alert_upload_bucket']}/{path_alert}",index=False)
    # LOGGER.info("Saved the alerts_dataframe in S3.")

    return {
        "status":"200",
        "Message":"Missing JSONs generated successfully"
    }