...
def upload_to_s3(prod_bucket, upload_path, local_path, gen_date, conf):
    ...
    s_3.Object(prod_bucket, upload_path).put(Body=open(local_path,'rb'), \
        Metadata={'Generation_Date':gen_date, 'Forecast_type': conf.get_forecast_type()})
    print(prod_bucket)
    print(upload_path)
...
def get_location_data_limits(connection, conf, weekday):
    ...
    query_dollarsales = open('core_forecast_csvtojson_redundancy/data_limits.sql', 'r').read()
    ...
    query_transcount = open('core_forecast_csvtojson_redundancy/data_limits.sql', 'r').read()
    ...
    query_itemcount = open('core_forecast_csvtojson_redundancy/data_limits.sql', 'r').read()
    ...
    query_ingredient = open('core_forecast_csvtojson_redundancy/data_limits.sql', 'r').read()
...
def lambda_handler(event, context):
    ...
    with open('core_forecast_csvtojson_redundancy/config.json') as config_params:
    ...
    query_dates = open('core_forecast_csvtojson_redundancy/get_dates.sql', 'r')
    dates = query_dates.read()
    business_dates = get_business_date(dates, connection, conf.get_database(), \
        conf.get_15min_dollar_table(), conf.get_store_num(), 'fetchdates')

    for dates in business_dates:
        
        lastyear_date = open('core_forecast_csvtojson_redundancy/get_last_year_date.sql', 'r')
        ...
        daily_dollartranscount = open('core_forecast_csvtojson_redundancy/daily_dollartranscount.sql', 'r').read()
        ...
        dollartranscount_daily = execute_query(daily_dollartranscount, connection, \
            conf.get_database(), conf.get_store_num(), conf.get_daily_dollartranscount_columns(), \
        ...
        if len(store_list) < 1:
            ...
            qry_15min_dollartranscount = open('core_forecast_csvtojson_redundancy/15min_dollartranscount.sql', 'r').read()
            ...
            dollartranscount_15min = execute_query(qry_15min_dollartranscount, connection, \
                conf.get_database(), conf.get_store_num(), \
            ...
            qry_15min_itemcount = open('core_forecast_csvtojson_redundancy/15min_itemcount.sql', 'r').read()
            ...
            itemcount_15min = execute_query(qry_15min_itemcount_final, connection, \
                conf.get_database(), conf.get_store_num(), conf.get_15min_itemcount_columns(), \
                conf.get_to_str_15min_itemcount_columns())
            LOGGER.info('15min forecast for Itemcount is done for'+conf.get_store_num())

            qry_15min_ingredient = open('core_forecast_csvtojson_redundancy/15min_ingredient.sql', 'r').read()
            ...
            ingredient_15min = execute_query(qry_15min_ingredient, connection, \
                conf.get_database(), conf.get_store_num(), conf.get_15min_ingredient_columns(), \
    ...
    for dates in range(len(business_dates)):
        try :
            ...
            if status == 'Success':
                ...
                business_dates[dates].strftime("%m-%d-%Y")+'.json', \
                conf.get_summaryjson_local_path()+date+'.json', gen_date_summary_forecast, conf)
                LOGGER.info("JSON QC failed and uploaded to test bucket")
                
            LOGGER.info("JSON upload completed for"+str(conf.get_store_num()))

        except :
            print("There is no data to fetch")
            import traceback
            traceback.print_exc()
