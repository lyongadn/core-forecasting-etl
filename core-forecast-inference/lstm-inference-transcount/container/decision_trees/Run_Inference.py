...
class Inference():
    def __init__(self):
        print("class init")
        with open('config_transcount_inference.json') as f:
...
#                print(outlier_record)
                ...
                #target_dir = '/tmp/'
                transfer.upload_file(pred_fname, str(self.config["bucket"]), "outliers_record/transcount/"+pred_fname)
...
#                records_df = df[df['outliers']==True, ['location_num', 'business_date', 'dayofweek', 'dayofweek_median_col', colname, 'dayofweek_median_col']]
    ...
    def nthDayPrediction(self, store_num, PredictedData, nthStartDayDate, nthEndDayDate,firstDayDate,Pacific):
            ...
            nthDayTranscount = PredictedData.loc[nthStartDayDateStr: nthEndDayDateStr, self.config["metric"]]

            # Saving the predictions
            df = pd.DataFrame(columns=['location_num', 'businessdate', 'forecast','generation_date'], \
                  data= {'location_num':str(store_num),
                         'businessdate':nthDayTranscount.index,
                         'forecast': nthDayTranscount.values,
    ...
    def sales_inference(self, store_num,bucket_name,athena_csv_filepath,mp_path,h5_path,upload_path):
        ...
        bucket_mp = bucket
        prop_fname = 'model-object-retraining-jan21/'+str(i)+'_transcount_allproducts/'+str(i)+'_transcount_allproducts.json'
        print (prop_fname)
        ...
        bucket_m = bucket
        model_fname = 'model-object-retraining-jan21/'+str(i)+'_transcount_allproducts/'+str(i)+'_transcount_allproducts.h5'
        print (model_fname)
        result = s3.download_file(bucket_m, model_fname, '/tmp/'+str(i)+'_transcount_allproducts.h5')
        multi_model = load_model('/tmp/'+str(i)+'_transcount_allproducts.h5')
        os.remove('/tmp/'+str(i)+'_transcount_allproducts.h5')
        #**************************************# 
