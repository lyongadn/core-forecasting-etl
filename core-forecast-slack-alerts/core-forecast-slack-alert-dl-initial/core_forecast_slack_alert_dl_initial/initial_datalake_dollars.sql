select distinct qcdata.* from (select location_num,dailyqc from store_wise_qc.dataprep_initial_dollarsalesandtranscount_datalake 
where generation_date = (select max(generation_date)
from store_wise_qc.dataprep_initial_dollarsalesandtranscount_datalake)) qcdata
inner join ml_preprod.crawler_stores_fixed totalstores on qcdata.location_num = totalstores.location_num	 
inner join (select distinct location_num from store_wise_qc.dataprep_initial_dollarsandtranscount_daily
where generation_date= (select max(generation_date) from store_wise_qc.dataprep_initial_dollarsandtranscount_daily)
)datalakeupdation
