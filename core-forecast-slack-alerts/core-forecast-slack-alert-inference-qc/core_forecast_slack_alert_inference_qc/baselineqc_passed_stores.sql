...
from 
   (select items.location_num from 
...
inner join (
   select distinct location_num from baseline_qc.forecast_15min_dollarsandtrans_14thday_ahead where 
   generation_date = (select max(generation_date) from baseline_qc.forecast_15min_dollarsandtrans_14thday_ahead)
   and forecast_qc=1
   union
   select distinct location_num from baseline_qc.forecast_15min_dollarsandtrans_30thday_ahead where 
   generation_date = (select max(generation_date) from baseline_qc.forecast_15min_dollarsandtrans_30thday_ahead)
   and forecast_qc=1
   )dollars 
on items.location_num =dollars.location_num)final
where location_num not in 
   (select distinct location_num from baseline_qc.forecast_15min_item_14thday_ahead where 
      generation_date = (select max(generation_date) from baseline_qc.forecast_15min_item_14thday_ahead)
      and forecast_qc=0
      union
      ...
      and forecast_qc=0
      union 
      select distinct location_num from ml_preprod.lstm_baseline_location_list
      where forecast_type = 'LSTM'
   );
